...
//"//a[@title-'My Profile']"
	...
	@FindBy(id = "cropWaitingPage:croppingFrom")
	public WebElement spinnerWhileCropping;

	@FindBy(id = "progressIcon")
	public WebElement fileUplasSpinner;
...
//used
	public boolean verifyEditContactPopUp(WebDriver driver) {
		boolean isCContactPopUpTabVerified = false;
		if (CommonUtils.waitForElement(driver, iframeAboutTab)) {
			CommonUtils.frameById(driver, iframeAboutTab);
			// driver.switchTo().frame(iframeAboutTab);
			if (aboutTab.isDisplayed() && contactTab.isDisplayed()) {
				isCContactPopUpTabVerified = true;
			}
			driver.switchTo().parentFrame();
		}
		return isCContactPopUpTabVerified;
	}
...
//used
	public boolean verifyLastNameChangeInAboutTab(WebDriver driver, String sLastName) throws InterruptedException {
		...
		if (aboutTab.isDisplayed()) {
			...
			CommonUtils.frameSwitchToParent(driver);
			// driver.switchTo().parentFrame();
			...
			System.out.println("AbouTab is not displayed");
		}
...
//used
	public boolean verifyCreatePost(WebDriver driver, String sPostMessage) throws InterruptedException {
		...
		if (CommonUtils.waitForElement(driver, postLink)) {
			postLink.click();
			CommonUtils.frameByIndex(driver, 0);
...
//used
	public boolean uploadPhoto(WebDriver driver, String sFilePath) throws InterruptedException {
		...
		CommonUtils.frameById(driver, photoUploadIframe);
